...
from wechat.sdk.messages import MESSAGE_TYPES, UnknownMessage
from wechat.sdk.exceptions import ParseError, NeedParseError, NeedParamError, \
    OfficialAPIError
from wechat.sdk.reply import TextReply, ImageReply, VoiceReply, VideoReply, MusicReply, \
    Article, ArticleReply
from wechat.sdk.lib import disable_urllib3_warning, XMLStore
...
class WechatBasic(object):
    ...
    def get_access_token(self):
        """
        获取 Access Token 及 Access Token 过期日期, 仅供缓存使用,
        如果希望得到原生的 Access Token 请求数据请使用 :func:`grant_token`
    ...
    def get_session_key(self, code):
        """
        获取小程序 session_key， 同时获取到 openid
        :param 为小程序登录后返回的 code 值
        :return:
            成功：
            {
                "openid": "OPENID",
    ...
    def get_jsapi_ticket(self):
        """
        获取 Jsapi Ticket 及 Jsapi Ticket 过期日期, 仅供缓存使用,
        如果希望得到原生的 Jsapi Ticket 请求数据请使用 :func:`grant_jsapi_ticket`
    ...
    def response_video(self, media_id, title=None, description=None):
        ...
        return VideoReply(message=self.__message, media_id=media_id,
                          title=title, description=description).render()

    def response_music(self, music_url, title=None, description=None,
        ...
        return MusicReply(message=self.__message, title=title,
                          description=description, music_url=music_url,
                          hq_music_url=hq_music_url,
                          thumb_media_id=thumb_media_id).render()

    def response_news(self, articles):
        """
        将新闻信息组装为符合微信服务器要求的响应数据
        :param articles: list 对象, 每个元素为一个 dict 对象,
        key 包含 `title`, `description`, `picurl`, `url`
        :return: 符合微信服务器要求的 XML 响应数据
        """
        self._check_parse()
    ...
    def grant_token(self, override=True):
        ...
        if override:
            cache.set("wechat_access_token", response_json,
                      timeout=response_json["expires_in"])
            self.__access_token = response_json['access_token']
            self.__access_token_expires_at = int(time.time()) + response_json[
                'expires_in']
        return response_json
    ...
    def grant_jsapi_ticket(self, override=True):
        """
        获取 Jsapi Ticket
        详情请参考 http://mp.weixin.qq.com/wiki/7/aaa137b55fb2e0456bf8dd9148dd613f.html
        ...
        if override:
            ...
            self.__jsapi_ticket_expires_at = int(time.time()) + response_json[
                'expires_in']
        return response_json
    ...
    def upload_media(self, media_type, media_file, extension=''):
        ...
        """
        self._check_appid_appsecret()
        if not isinstance(media_file, IOBase) and not isinstance(media_file,
                                                               StringIO):
            raise ValueError(
                'Parameter media_file must be file object or StringIO.StringIO object.')
        if isinstance(media_file, StringIO) and extension.lower() not in ['jpg',
                                                                          'jpeg',
                                                                          'amr',
                                                                          'mp3',
                                                                          'mp4']:
    ...
    def get_followers(self, first_user_id=None):
        ...
        return self._get('https://api.weixin.qq.com/cgi-bin/user/get',
                         params=params)
    ...
    def show_qrcode(self, ticket):
        ...
        """
        self._check_appid_appsecret()
        if "\\" in ticket:  # 兼容新版微信返回了 \\ 内容但接口并不认
    ...
    def send_wx_template_message(self, user_id, form_id, template_id, data,
                                 page='', color='#FF0000'):
        """
        发送小程序模版消息
        详情请参考 https://mp.weixin.qq.com/debug/wxadoc/dev/api/notice.html
        :param user_id: 用户 ID, 就是你收到的 WechatMessage 的 source (OpenID)
        :param template_id: 模板ID
        :param form_id: 小程序form ID
        :param data: 模板消息数据 (dict形式)，示例如下：
        {
            "first": {
               "value": "恭喜你购买成功！",
        ...
        :param page: 跳转地址 (默认为空)
        :param color: 顶部颜色RGB值 (默认 '#FF0000' )
        :return: 返回的 JSON 数据包
        """
        self._check_appid_appsecret()
    ...
    def access_token(self):
        self._check_appid_appsecret()
        token_dict = cache.get("wechat_access_token")
        if token_dict:
    ...
    def _request(self, method, url, **kwargs):
        ...
        r.raise_for_status()
        response_json = ast.literal_eval(r.content.decode())
